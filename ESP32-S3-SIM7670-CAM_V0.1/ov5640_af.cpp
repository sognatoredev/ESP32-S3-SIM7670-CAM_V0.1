#include "ov5640_af.h"
#include "esp_camera.h"
#include <Arduino.h>

// ── 레지스터 주소 ──────────────────────────────────────────────────────────────
#define OV5640_CMD_MAIN      0x3022  // AF 명령 입력
#define OV5640_CMD_ACK       0x3023  // AF 명령 수락 확인
#define OV5640_CMD_FW_STATUS 0x3029  // AF 펌웨어 상태

// ── AF 명령값 ──────────────────────────────────────────────────────────────────
#define AF_CMD_SINGLE        0x03    // 단발 AF (SAF)
#define AF_CMD_CONTINUOUS    0x04    // 연속 AF (CAF)
#define AF_CMD_PAUSE         0x08    // AF 일시정지

// ── 펌웨어 상태값 ──────────────────────────────────────────────────────────────
#define AF_FW_IDLE           0x70    // MCU 준비 완료
#define AF_FW_FOCUSED        0x10    // 포커스 완료
#define AF_FW_FOCUSING       0x00    // 포커스 탐색 중 (0x00~0x0F)
#define AF_FW_OUT_OF_FOCUS   0x20    // 초점 불가

// ── OV5640 AF 펌웨어 (3,584 bytes) ────────────────────────────────────────────
// MIT License: ESP32-OV5640-AF library by Eric Nam (2021)
// Source: https://github.com/0015/ESP32-OV5640-AF
static const uint8_t AF_FW[] = {
    ,
};

// ── 내부 상태 ──────────────────────────────────────────────────────────────────
static sensor_t *s_sensor = nullptr;
static bool      s_fwLoaded = false;

// ── 구현 ───────────────────────────────────────────────────────────────────────

int ov5640AfInit()
{
    s_sensor = esp_camera_sensor_get();
    if (!s_sensor) {
        Serial.println("[AF] No sensor");
        return -1;
    }

    // OV5640 PID 확인 (0x5640)
    uint8_t vid = s_sensor->get_reg(s_sensor, 0x300a, 0xff);
    uint8_t pid = s_sensor->get_reg(s_sensor, 0x300b, 0xff);
    if (vid != 0x56 || pid != 0x40) {
        Serial.printf("[AF] Not OV5640 (vid=0x%02X pid=0x%02X) — AF skipped\n", vid, pid);
        return -2;
    }

    Serial.println("[AF] Loading firmware...");
    uint32_t t0 = millis();

    // 8051 MCU 리셋
    s_sensor->set_reg(s_sensor, 0x3000, 0xff, 0x20);

    // 펌웨어 쓰기 (0x8000~)
    uint16_t addr = 0x8000;
    for (size_t i = 0; i < sizeof(AF_FW); i++) {
        if (s_sensor->set_reg(s_sensor, addr++, 0xff, AF_FW[i]) < 0) {
            Serial.printf("[AF] FW write error at offset %u\n", i);
            return -3;
        }
    }

    // 명령 레지스터 초기화
    s_sensor->set_reg(s_sensor, OV5640_CMD_MAIN,      0xff, 0x00);
    s_sensor->set_reg(s_sensor, OV5640_CMD_ACK,       0xff, 0x00);
    s_sensor->set_reg(s_sensor, 0x3024,               0xff, 0x00);
    s_sensor->set_reg(s_sensor, 0x3025,               0xff, 0x00);
    s_sensor->set_reg(s_sensor, 0x3026,               0xff, 0x00);
    s_sensor->set_reg(s_sensor, 0x3027,               0xff, 0x00);
    s_sensor->set_reg(s_sensor, 0x3028,               0xff, 0x00);
    s_sensor->set_reg(s_sensor, OV5640_CMD_FW_STATUS, 0xff, 0x7f);

    // MCU 리셋 해제
    s_sensor->set_reg(s_sensor, 0x3000, 0xff, 0x00);

    // IDLE(0x70) 대기 (최대 5초)
    for (int i = 0; i < 1000; i++) {
        uint8_t st = s_sensor->get_reg(s_sensor, OV5640_CMD_FW_STATUS, 0xff);
        if (st == AF_FW_IDLE) {
            s_fwLoaded = true;
            Serial.printf("[AF] FW ready (%lu ms, %u bytes)\n",
                          millis() - t0, sizeof(AF_FW));
            return 0;
        }
        delay(5);
    }

    Serial.println("[AF] FW init timeout");
    return -4;
}

int ov5640AfTriggerSingle()
{
    if (!s_sensor || !s_fwLoaded) return -1;

    uint16_t retry = 0;

    // 이전 동작 정지 — FW_STATUS가 IDLE(0x70)로 돌아올 때까지 대기 (최대 1초)
    // CMD_ACK 대신 FW_STATUS를 폴링해야 함:
    //   ov5640AfInit() 이후 CMD_ACK는 이미 0x00이므로 ACK 폴링은 즉시 반환되어
    //   MCU가 실제로 PAUSE를 처리했는지 확인할 수 없다.
    s_sensor->set_reg(s_sensor, OV5640_CMD_MAIN, 0xff, AF_CMD_PAUSE);
    while (s_sensor->get_reg(s_sensor, OV5640_CMD_FW_STATUS, 0xff) != AF_FW_IDLE) {
        if (++retry > 200) break;   // 타임아웃 시에도 계속 진행
        delay(5);
    }

    // SAF 트리거
    retry = 0;
    s_sensor->set_reg(s_sensor, OV5640_CMD_ACK,  0xff, 0x01);
    s_sensor->set_reg(s_sensor, OV5640_CMD_MAIN, 0xff, AF_CMD_SINGLE);
    while (s_sensor->get_reg(s_sensor, OV5640_CMD_ACK, 0xff) != 0x00) {
        if (++retry > 500) return -3;
        delay(5);
    }

    return 0;
}

int ov5640AfTriggerContinuous()
{
    if (!s_sensor || !s_fwLoaded) return -1;

    uint16_t retry = 0;

    // 이전 동작 정지 — FW_STATUS가 IDLE(0x70)로 돌아올 때까지 대기 (최대 1초)
    s_sensor->set_reg(s_sensor, OV5640_CMD_MAIN, 0xff, AF_CMD_PAUSE);
    while (s_sensor->get_reg(s_sensor, OV5640_CMD_FW_STATUS, 0xff) != AF_FW_IDLE) {
        if (++retry > 200) break;   // 타임아웃 시에도 계속 진행
        delay(5);
    }

    // CAF 트리거
    retry = 0;
    s_sensor->set_reg(s_sensor, OV5640_CMD_ACK,  0xff, 0x01);
    s_sensor->set_reg(s_sensor, OV5640_CMD_MAIN, 0xff, AF_CMD_CONTINUOUS);
    while (s_sensor->get_reg(s_sensor, OV5640_CMD_ACK, 0xff) != 0x00) {
        if (++retry > 500) return -3;
        delay(5);
    }

    return 0;
}

bool ov5640AfWaitFocus(uint32_t timeoutMs)
{
    if (!s_sensor || !s_fwLoaded) return false;

    uint32_t start = millis();
    while (millis() - start < timeoutMs) {
        uint8_t st = s_sensor->get_reg(s_sensor, OV5640_CMD_FW_STATUS, 0xff);
        if (st == AF_FW_FOCUSED) {
            Serial.printf("[AF] Focused in %lu ms\n", millis() - start);
            return true;
        }
        if (st == AF_FW_OUT_OF_FOCUS) {
            Serial.println("[AF] Out of focus range");
            return false;
        }
        delay(20);
    }
    Serial.printf("[AF] Focus timeout (%lu ms)\n", timeoutMs);
    return false;
}

uint8_t ov5640AfGetStatus()
{
    if (!s_sensor) return 0xFF;
    return s_sensor->get_reg(s_sensor, OV5640_CMD_FW_STATUS, 0xff);
}
